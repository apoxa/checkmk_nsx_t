...
def check_nsx_vservers(item: str, section: SECTION) -> CheckResult:
    ...
    enabled_state, enabled_txt = _ENABLED_MAP.get(vserver['enabled'], 
                                                 (3, "unknown[%s]" % vserver['enabled']))
    yield Result(state=enabled_state, summary=f"is {enabled_txt}")

    yield Result(state=_STATUS_MAP.get(vserver['status'], 3), summary=f"State: {vserver['status']}")

    yield Result(state=State.OK, summary=f"ID: {vserver['id']}") 


def cluster_check_nsx_vservers(item: str, section: Mapping[str, SECTION]) -> CheckResult:
    datasets, nodeinfos = [], []
    ...
    if len(set([y for x in list(map(lambda x: list(x.values()), datasets)) for y in x])) > len(set(list(datasets[0].values()))):
